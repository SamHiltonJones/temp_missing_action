...
def load_data(dataset_path):
    with open(dataset_path, 'rb') as file:
        data_dict = pickle.load(file)  # Load the single object, which is your data dictionary

    if 'pkl' in data_dict:
        data_dict = data_dict['pkl']
    else:
        # Optionally handle the case where the dictionary does not have a 'pkl' key
        print("No 'pkl' key in data dictionary")

    # Debug: print out shapes to verify correct loading
    ...
    for y in range(grid_shape[0]):
        for x in range(grid_shape[1]):
            if not env.is_wall(x, y):
                ...
                state_values[y, x] = np.nan  # Walls, not accessible

    # Plotting the best actions
